...
#include <wasm-rt.h>
#include "add.h"  // Generated by wasm2c
#include <stdlib.h>
#include <stdio.h>
...
Wasm2cEngine* wasm2c_engine_new(void) {
    ...
    engine->instance = calloc(1, sizeof(struct w2c_add));
    if (!engine->instance) {
        free(engine);
        return NULL;
    }
    ...
    wasm2c_add_instantiate(engine->instance);
...
void wasm2c_engine_delete(Wasm2cEngine* engine) {
    if (!engine) return;
    if (engine->instance) {
        wasm2c_add_free(engine->instance);
...
float wasm2c_engine_get_sample(Wasm2cEngine* engine) {
    ...
    // The signature is: f32 w2c_add_get_sample(struct w2c_add*, f32)
    float dummy = 0.0f;
    float result = w2c_add_get_sample(engine->instance, dummy);
